...
class MySentences(object):
    ...
    def __iter__(self):
        ...
        for uid,line in enumerate(open(self.filename)):
            ...
            if self.dtype == 'train':
                # train:读入标签
                title = getLabel(article[0].decode('utf-8'))
                content = article[1:]
            elif self.dtype == 'test':
                # test:读入文档ID
                title = article[0].decode('utf-8')
                content = article[1:]
            elif self.dtype == 'all':
                content = article[0:]
            # 把list连接成string
...
def train_model(train_file,pred_file,text_file,model_file):
    ...
    model_dm = Doc2Vec(min_count=5, window=15, size=SIZE, sample=1e-3, negative=5, workers=4,alpha=0.025, min_alpha=0.025)
    model_dbow = Doc2Vec(min_count=5, window=15, size=SIZE, sample=1e-3, negative=5, dm=0, workers=4,alpha=0.025, min_alpha=0.025)
...
def transform_doc2vec(model_file,train_file,test_file):
    ...
    model_dm.wv.load_word2vec_format(dm_bin, binary=True)

    # 读入模型
    dbow_mdl = model_file + "_dbow.mdl"
    dbow_bin = model_file + "_dbow.bin"
    model_dbow = Doc2Vec.load(dbow_mdl)
    model_dbow.wv.load_word2vec_format(dbow_bin, binary=True)
...
def main():
    ...
    # 训练数据
    trainText =dataPath + "train/train_p5.txt"

    modelFile = mdlPath + "doc2vec_p5"

    # 测试数据
    testText = dataPath + "test/test_all.tsv"

    # 测试数据
    # cut - c19 - train / train_p1.txt >./ text_all.txt
    # os.system("cut - c19 - train / train_p1.txt >./ text_all.txt")

    # cut - c33 - test / test_p1.txt >>./ text_all.txt
    # os.system("cut - c19 - train / train_p1.txt >./ text_all.txt")
