...
# 11-22 jieba分词使用并行模式提高速度
# 11-22 jieba分词结果引入词性标注结果
...
# 去除标点符号和特殊符号
def scan(line):
    string = re.sub("[+\:;?\"\.\!\/▶_,$%^*(+\'——！℃，“”·\-▲▼▽◆\|•★ˇ<>「」。？、﹏『』© 《》�♡‘’⑧《~@#￥%……&*（）丨×〔〕■▪→↓☆ ▁ ▂ ▃ ▄ ▅ ▆ ● ҈]+".decode("utf8"),"".decode("utf8"),line.decode("utf8"))
...
def readDataFile(fileName,ptype):
    ...
    # 读入停用词
    swFile = "/home/hadoop/DataSencise/bdci2017/BDCI2017-360-Semi/stopwords.txt"
    ssList = readStopWord(swFile)
...
def main():
    ...
    # 处理训练文件
    run(input_train,train_outfile_all,'train')
    
    # 处理测试文件
    # run(input_test,test_outfile_all,'test')
